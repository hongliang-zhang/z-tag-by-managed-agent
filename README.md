...
基于 Claude Tag 官方资料、42 张真实产品截图与 Claude Managed Agents 的公开信息，拆解团队级 Agent 产品，并给出以 GLM Managed Agent 为执行底座的产品与技术架构方案。

> 核心结论：Claude Tag 不是“每频道一个 Agent”，而是“一个组织级 Agent + Scope Graph + Access Bundle + 每 Thread 一个 Session”。GLM Managed Agent 负责把任务做完；Z Tag 负责把频道、配置、身份权限和团队治理编译成可执行的 Session。
