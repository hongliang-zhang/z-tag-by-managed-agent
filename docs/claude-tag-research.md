...
> 研究快照：2026-08-11。本文优先使用 Anthropic 官方文档；真实产品截图用于补充界面和操作细节；YouTuber 解读仅作为架构假设来源，不作为 Anthropic 内部实现的证据。
...
3. Thread 中显示接单、Checklist、运行状态以及 “Open session in Claude” 等入口。
4. 任何频道成员都可以在同一 Thread 中补充、纠偏或改变任务，无需再次 `@Claude`。
5. Claude 将结果、文件、PR 或链接回帖到原 Thread。
...
- Audit 页面实际包含 Scheduled work、Memory、Network events 三类，而不是一张完整的逐动作审计表。
...
4. Effective Config Resolver 按“覆盖、累加、拼接、冲突优先级”生成 `SessionLaunchSpec`。
...
| 长期状态 | Channel/Workspace Memory、Routine | Memory Store、Cron/Webhook 原语 |
